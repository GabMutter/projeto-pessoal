...
class Evento {
  ...
  static async create(data) {
    const result = await db.query(
      ...
      [data.titulo, data.confirmar, data.data_inicio, data.data_final, data.prioridade, data.id_usuario, data.id_endereco]
    );
    return result.rows[0];
  }
  ...
  static async update(id, data) {
    const result = await db.query(
      ...
      [data.titulo, data.confirmar, data.data_inicio, data.data_final, data.prioridade, data.id_usuario, data.id_endereco, id]
    );
    return result.rows[0];
  }
